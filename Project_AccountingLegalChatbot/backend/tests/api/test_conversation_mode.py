...
@pytest.mark.asyncio
async def test_new_conversation_defaults_to_fast_mode(client):
...
@pytest.mark.asyncio
async def test_list_includes_mode(client):
...
@pytest.mark.asyncio
async def test_patch_mode_updates_and_persists(client):
...
@pytest.mark.asyncio
async def test_patch_mode_rejects_invalid_value(client):
